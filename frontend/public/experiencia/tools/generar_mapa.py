...
# La raiz del repo es la primera carpeta hacia arriba que tenga `backend/`:
# la experiencia vive en frontend/public/experiencia, tres niveles abajo.
RAIZ = EXPERIENCIA
while not os.path.isdir(os.path.join(RAIZ, "backend")) and os.path.dirname(RAIZ) != RAIZ:
    RAIZ = os.path.dirname(RAIZ)
# El backend se reestructuro en paquetes (backend/Model, backend/scraping) y
# este script se quedo apuntando a la raiz, donde vivian antes: `catalogos.py`,
# `scraper_projects.py` y `datos/`. Se agregan las dos carpetas al path en vez
# de reescribir los imports para que siga corriendo tal cual desde tools/.
BACKEND = os.path.join(RAIZ, "backend")
sys.path.insert(0, RAIZ)
sys.path.insert(0, os.path.join(BACKEND, "Model"))
sys.path.insert(0, os.path.join(BACKEND, "scraping"))

from catalogos import LOCALIDADES_BOGOTA, normalizar_texto  # noqa: E402
import scraper_projects as sp  # noqa: E402

GEO = os.path.join(BACKEND, "Model", "data_projects", "localidades_bogota.json")
SALIDA = os.path.join(EXPERIENCIA, "js", "mapa_bogota.js")
SALIDA_BARRIOS = os.path.join(EXPERIENCIA, "js", "mapa_barrios.js")
SALIDA_LUGARES = os.path.join(EXPERIENCIA, "js", "mapa_lugares.js")
...
CACHE_OSM_LUGARES = os.path.join(AQUI, ".cache_osm_lugares.json")
...
def _consultar_overpass(consulta):
    """Le pregunta a Overpass y devuelve el JSON crudo.
...
# Claves de OSM que marcan "sitio con nombre que alguien podria usar para
# ubicarse". Se pide por CLAVE y no por valor (shop=mall, shop=supermarket...)
# porque enumerar valores se queda corto siempre: lo que buscamos es cualquier
# cosa con nombre que una persona reconozca al pasar por la calle.
CLAVES_LUGAR = ("shop", "amenity", "leisure", "tourism", "office")

# QUE ENTRA AL CATALOGO LOCAL, y por que es una lista corta y no un catch-all.
#
# Se probo pidiendo TODO lo que tuviera nombre bajo esas cinco claves: salieron
# 33.952 sitios y 1,75 MB, con 3.804 restaurantes, 1.190 panaderias y nombres
# como "--------" o "El Buen Gusto". Ninguna panaderia de barrio sirve para
# ubicar una zona, y en el buscador empujan hacia abajo al centro comercial que
# si se estaba buscando.
#
# La cola larga NO se pierde: la cubre el geocodificador en vivo (ver
# PHOTON_URL en js/main.js), que busca sobre todo OSM. Este catalogo tiene otro
# trabajo — ser la base instantanea y a prueba de wifi caido — y para eso lo
# que hace falta son los sitios que cualquiera reconoce al pasar.
VALORES_LUGAR = {
    # comercio grande, del que la gente usa como referencia
    "mall", "department_store", "supermarket", "marketplace",
    # verde y deporte
    "park", "stadium", "sports_centre", "pitch", "golf_course",
    # educacion
    "university", "college", "school", "kindergarten", "library",
    # salud
    "hospital", "clinic",
    # cultura y culto
    "place_of_worship", "museum", "theatre", "cinema", "arts_centre",
    "attraction", "zoo", "theme_park", "monument", "memorial", "viewpoint",
    # transporte y civico
    "bus_station", "station", "terminal", "townhall", "embassy", "police",
    "fire_station", "courthouse", "prison", "hotel",
}

# Por debajo de esto un nombre no distingue nada ("Bar", "Cafe", "SAO").
LARGO_MINIMO_LUGAR = 4


def descargar_lugares_osm():
    """Sitios con nombre dentro de OSM_BBOX: centros comerciales, parques,
    universidades, colegios, hospitales, iglesias, museos, estaciones y
    comercios en general. [{"n": nombre, "lat": .., "lon": .., "cat": ..}].

    Es el catalogo del segundo buscador de la pregunta de ubicacion, el de
    "se un lugar cerca". Con cache en disco: borrar
    `tools/.cache_osm_lugares.json` fuerza la descarga.

    `out center` es OBLIGATORIO: la mayoria de estos sitios estan mapeados
    como `way` o `relation` (un centro comercial es un poligono, no un punto)
    y esos no traen lat/lon propios — solo un `center` cuando se pide.
    """
    if os.path.isfile(CACHE_OSM_LUGARES):
        with open(CACHE_OSM_LUGARES, encoding="utf-8") as fh:
            return json.load(fh)

    partes = "".join(
        '%s["%s"]["name"];' % (tipo, clave)
        for clave in CLAVES_LUGAR
        for tipo in ("node", "way", "relation")
    )
    respuesta = _consultar_overpass(
        '[out:json][timeout:300][bbox:%s];(%s);out center;' % (OSM_BBOX, partes)
    )

    # EL CACHE GUARDA TODO LO QUE TRAJO OSM, sin filtrar. Lo caro es la
    # descarga (son ~40.000 sitios y Overpass tarda minutos); el filtro es
    # instantaneo. Guardando el crudo se puede cambiar `VALORES_LUGAR` y
    # regenerar sin volver a pedirle nada a Overpass.
    lugares = []
    for el in respuesta.get("elements") or []:
        tags = el.get("tags") or {}
        ...
        if not nombre:
            continue
        # De las cinco claves posibles se guarda la primera que traiga, con su
        # valor: es la categoria del sitio y con ella se filtra despues.
        cat = ""
        for clave in CLAVES_LUGAR:
            if tags.get(clave):
                cat = tags[clave]
                break
        centro = el.get("center") or el
        if "lat" not in centro or "lon" not in centro:
            continue
        lugares.append({"n": nombre, "lat": centro["lat"], "lon": centro["lon"], "cat": cat})

    with open(CACHE_OSM_LUGARES, "w", encoding="utf-8") as fh:
        json.dump(lugares, fh, ensure_ascii=False)
    return lugares


def filtrar_lugares(crudos):
    """Se queda con los sitios que sirven para ubicarse (ver `VALORES_LUGAR`).

    Aparte del tipo, se descartan dos formas de nombre inutil: los demasiado
    cortos para distinguir nada y los que no traen ni una letra ("--------",
    "...", "1"), que nadie podria escribir en el buscador.
    """
    return [
        l for l in crudos
        if l.get("cat") in VALORES_LUGAR
        and len(l["n"]) >= LARGO_MINIMO_LUGAR
        and any(c.isalpha() for c in l["n"])
    ]
...
def resolver_lugares(lugares, originales_loc, barrios_geo):
    """Cada lugar a la zona que le corresponde: `(resueltos, descartados)`.

    Resueltos son `{n, cat, loc, bi}`, donde `loc` es el id 1..20 de localidad
    y `bi` la posicion del sector dentro de `barrios_geo[loc]` — las mismas
    dos coordenadas con las que ya viaja cada entrada del buscador de barrios
    (ver `GDF_BARRIOS`), asi que el front las resuelve por el camino de
    siempre y no hace falta mandarle lat/lon.

    SE RESUELVE AQUI Y NO EN EL NAVEGADOR porque este es el unico sitio donde
    ya estan los poligonos originales: hacerlo en build-time deja el archivo
    generado mas liviano y el buscador instantaneo. Los que llegan del
    geocodificador en vivo si se resuelven en el navegador, contra los mismos
    contornos (ver `zonaEnPunto` en js/mapa.js) — son los dos unicos caminos y
    dan lo mismo.

    LA LOCALIDAD SE DECIDE CONTRA LOS ANILLOS ORIGINALES, igual que en
    `procesar_barrios`: los simplificados mueven el limite hasta 33 m, y un
    centro comercial pegado a una frontera se iria a la localidad de al lado.

    Un lugar fuera de las 20 localidades (Chia, Soacha, una coordenada rota)
    se descarta y se reporta: no llega al navegador, asi que no hay forma de
    que alguien elija algo que no se puede ubicar.
    """
    resueltos = []
    descartados = []
    # Una caja por localidad para descartar rapido: son ~40.000 lugares contra
    # 20 poligonos de miles de vertices, y el ray casting completo sobre todos
    # seria la parte lenta de todo el script.
    cajas = {}
    for idx, anillos in originales_loc.items():
        xs = [p[0] for a in anillos for p in a]
        ys = [p[1] for a in anillos for p in a]
        cajas[idx] = (min(xs), min(ys), max(xs), max(ys))

    for lugar in lugares:
        x, y = lugar["lon"], lugar["lat"]
        idx_loc = None
        for idx, anillos in originales_loc.items():
            c = cajas[idx]
            if x < c[0] or x > c[2] or y < c[1] or y > c[3]:
                continue
            if dentro(x, y, anillos):
                idx_loc = idx
                break
        if idx_loc is None:
            descartados.append(lugar["n"])
            continue

        # El sector dentro de esa localidad. Puede no haberlo (huecos entre
        # sectores, zona rural): ahi `bi` queda None y vale la localidad sola,
        # que es el mismo caso que ya maneja un clic en un hueco del mapa.
        bi = None
        for i, b in enumerate(barrios_geo.get(idx_loc, [])):
            bb = b["bb"]
            if x < bb[0] or x > bb[2] or y < bb[1] or y > bb[3]:
                continue
            if dentro(x, y, b["anillos"]):
                bi = i
                break
        resueltos.append({"n": lugar["n"], "cat": lugar.get("cat", ""), "loc": idx_loc, "bi": bi})

    return resueltos, descartados


def dedupe_lugares(lugares):
    """Un sitio, una entrada. OSM repite el mismo lugar como nodo y como
    poligono, y las cadenas ponen el mismo nombre en cada sede.

    Se deduplica por (nombre normalizado, localidad): dos sedes de la misma
    cadena en localidades distintas son dos respuestas utiles —"Exito de
    Suba" y "Exito de Kennedy" ubican en sitios distintos— pero dos entradas
    identicas en la misma localidad son la misma sugerencia repetida.
    """
    vistos = {}
    salida = []
    for lugar in lugares:
        clave = (normalizar_texto(lugar["n"]), lugar["loc"])
        if clave in vistos:
            continue
        vistos[clave] = True
        salida.append(lugar)
...
def main() -> int:
    ...
    # ---- EL CATALOGO DE LUGARES, el segundo buscador de la pregunta de zona.
    # Va despues de los barrios porque necesita sus poligonos para resolver en
    # que sector cae cada sitio.
    lugares_crudos = filtrar_lugares(descargar_lugares_osm())
    lugares_res, lugares_fuera = resolver_lugares(lugares_crudos, originales, barrios_geo)
    lugares = dedupe_lugares(lugares_res)
    lugares.sort(key=lambda l: normalizar_texto(l["n"]))

    filas_l = [
        "{n:%s,cat:%s,loc:%d%s}" % (
            json.dumps(l["n"], ensure_ascii=False),
            json.dumps(l["cat"], ensure_ascii=False),
            l["loc"],
            "" if l["bi"] is None else ",bi:%d" % l["bi"],
        )
        for l in lugares
    ]
    js_l = (
    ...
    with open(SALIDA_LUGARES, "w", encoding="utf-8") as fh:
        fh.write(js_l)
    ...
    _cats = {}
    for _l in lugares:
        _cats[_l["cat"]] = _cats.get(_l["cat"], 0) + 1
    _top = sorted(_cats.items(), key=lambda kv: -kv[1])[:6]
    print("lugares     : %d sitios (%d de OSM, %d duplicados, %d fuera de Bogota),"
          " %d sin sector; %d KB"
          % (len(lugares), len(lugares_crudos), len(lugares_res) - len(lugares),
             len(lugares_fuera), sum(1 for l in lugares if l["bi"] is None),
             os.path.getsize(SALIDA_LUGARES) // 1024))
    print("              mas comunes: %s"
          % ", ".join("%s %d" % (c or "(sin categoria)", n) for c, n in _top))
